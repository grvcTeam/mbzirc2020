        ...
        while changed:
            ...
            for i in range(nclusters):
                ...
                for j in range(i+1,nclusters):
                    ...
                    print('Checking clusters: ' + str(clusters[i]) + ',' + str(clusters[j]))
...
def main():
    ...
    while len(central_unit.uav_walls)==0 and not rospy.is_shutdown():
        print('No clustering')
        rospy.sleep(1.0)
    
    central_unit.cluster_wall_segments()
    print(central_unit.wall_segment_ids)
