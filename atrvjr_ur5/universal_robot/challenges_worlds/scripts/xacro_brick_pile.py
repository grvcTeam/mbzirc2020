...
parser = argparse.ArgumentParser(description='script to generate the xacro macro for a pile of bricks. The macro ' \
                                 'will be stored in the file urdf/[model_name].xacro')
...
def brick_string(id,mass,material,sx,sy,sz,x,y,z):
    s = '<brick_model id="{id}" type="brick" sx="{sx}" sy="{sy}" sz="{sz}" mass="{mass}" material="{material}" pose="{x} {y} {z} 0 0 0" />'.format(id=id, \
