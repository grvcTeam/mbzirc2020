...
public:
    ...
    ~CircleDetector();
private:
    void callbackCameraInfo(const sensor_msgs::CameraInfoConstPtr &camera_info_ptr);
    ...
    void findCirclesColorSim(const sensor_msgs::ImageConstPtr &color_img_ptr, const sensor_msgs::ImageConstPtr &depth_img_ptr);
    bool hasCameraInfo();
    ...
    // Depth and Color camera info subscriber
    ros::Subscriber camera_info_sub_1_;
    ros::Subscriber camera_info_sub_2_;
...
void CircleDetector::findCirclesColorSim(const sensor_msgs::ImageConstPtr &color_img_ptr, const sensor_msgs::ImageConstPtr &depth_img_ptr)
{
    // Convert from ROS image type to OpenCV
    cv_bridge::CvImagePtr color_image_cv = cv_bridge::toCvCopy(color_img_ptr, sensor_msgs::image_encodings::RGB8);
    cv_bridge::CvImagePtr depth_image_cv = cv_bridge::toCvCopy(depth_img_ptr, sensor_msgs::image_encodings::TYPE_32FC1);
    cv_bridge::CvImage gray_image_cv;
    gray_image_cv.encoding = sensor_msgs::image_encodings::MONO8;
    cv::cvtColor(color_image_cv->image, gray_image_cv.image, CV_BGR2GRAY);
    // Apply Blur
    ...
    for(i1 = 0; i1 < depth_image_cv->image.rows; i1++)
    {
        for(j1 = 0; j1 < depth_image_cv->image.cols; j1++)
        {
            const cv::Mat & original_image_ref_meters = depth_image_cv->image;
            const cv::Mat & mask_image_ref = mask_image_cv.image; 
            float * value_original_meters_ptr = (float *) (original_image_ref_meters.data + original_image_ref_meters.step[0]*i1 + original_image_ref_meters.step[1]*j1); 
            uint8_t * mask_image_pixel_ptr = (uint8_t *) (mask_image_ref.data + mask_image_ref.step[0] * i1 + mask_image_ref.step[1] * j1);
            uint32_t aux_integer;
            memcpy(&aux_integer, value_original_meters_ptr, sizeof(float));
            if ( aux_integer == 0X7FC00000 )
            {
                *mask_image_pixel_ptr = 0;
            }
            else if ( (*value_original_meters_ptr == 0.0) || (*value_original_meters_ptr > 10.0) || (*value_original_meters_ptr > max_depth_th_) )
            {
                *mask_image_pixel_ptr = 0;
            }
            else
    ...
    for (size_t i = 0; i < circles_detected.size(); i++)
    {
        ...
        //std::cout << "Circle " << i << " radius : " << radius << std::endl;
        // Extract circle center depth from depth image
        float circle_center_depth = (float)(depth_image_cv->image.at<float>(center));
        //std::cout << "Circle center depth: " << circle_center_depth << std::endl;
