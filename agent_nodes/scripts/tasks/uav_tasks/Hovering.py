...
class Task(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.uav_state == State.LANDED_ARMED:
            self.iface['cli_take_off'](TakeOffRequest(
                height=self.props['height'], blocking=True))
        elif self.uav_state == State.FLYING_AUTO:
            try:
                trans_global2uav = lookup_tf_transform(
                    self.props['global_frame'], self.props['agent_frame'], self.iface['tf_buffer'], 5)
            except Exception as error:
                print repr(error)
                print self.name + ' Task could not be executed'
                return 'error'

            pose = from_geom_msgs_Transform_to_geom_msgs_Pose(
                trans_global2uav.transform)
            pose.position.z = self.props['height']
            way = GoToWaypointRequest(waypoint=PoseStamped(
                header=Header(frame_id=self.props['global_frame'], stamp=rospy.Time.now()), pose=pose), blocking=True)
