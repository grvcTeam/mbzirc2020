...
void getInputParams(ros::NodeHandle nh){
  ...
  */
  ...
  if(nh.getParam("/target_drone_trajectory/speed", speed) &&
     nh.getParam("/target_drone_trajectory/a", a) &&
     nh.getParam("/target_drone_trajectory/b", b))
  {
    ROS_INFO("Parameters received --> a:[%f] b:[%f] speed:[%f]m/s",a,b,speed);
    if(speed > 10){
...
int main(int argc, char **argv)
{
  ...
  arm_service = nh.serviceClient<mavros_msgs::CommandBool>("mavros/cmd/arming");
  mode_service = nh.serviceClient<mavros_msgs::SetMode>("mavros/set_mode");
  model_state_service = nh.serviceClient<gazebo_msgs::GetModelState>("gazebo/get_model_state");
  pub = nh.advertise<geometry_msgs::PoseStamped>("mavros/setpoint_position/local",1000);

  ROS_INFO("MAIN ");

  // get drone initial pose
  getmodelstate_req.model_name = "iris";
